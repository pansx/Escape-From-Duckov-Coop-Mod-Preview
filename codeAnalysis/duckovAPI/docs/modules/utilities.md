...
1. CustomData 的键必须唯一，避免冲突
2. 数据类型一旦设置就不能更改，除非重新创建
3. CustomDataCollection 会自动管理内部字典，无需手动维护
4. PrefabPool 使用对象池模式，提高性能，避免频繁创建销毁对象
5. 对象池中的对象在释放后会被重置，确保下次使用时状态正确
6. 使用 IPoolable 接口可以实现对象池生命周期管理
