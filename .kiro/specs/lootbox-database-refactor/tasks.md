...
- [ ] 3. 实现主机端数据库初始化
  - [ ] 3.1 创建同步管理器类
    ...
    - 添加 `_hostDatabase` 字段（LootBoxDatabase 类型）
    - 添加 `_currentLootBox` 字段（客户端用）
    - 添加 `IsHost` 属性
    - 添加单例模式（可选）
    - _需求: 3.1_
  - [ ] 3.2 实现扫描战利品箱
...
- [ ] 12. 实现客户端拆分请求
  ...
  - 创建 `LootSplitRequest` 消息（SetId, SourcePosition, Count, PreferredPosition）
  - 通过 `JsonMessageRouter.Send()` 发送给主机
  - 记录日志：SetId、SourcePosition、Count、PreferredPosition
  - _需求: 2.4_
