...
- [ ] 1. 准备工作：验证和增强 PlayerInfoDatabase
...
- [ ] 2. 在 NetService 中添加数据库更新逻辑
  - [ ] 2.1 在 ClientStatusMessage 处理逻辑中更新数据库
  ...
  - [ ] 2.4 在 OnNetworkLatencyUpdate 中同步延迟到数据库
...
- [ ] 3. 重构 MModUI.UpdatePlayerList() 方法
  - [ ] 3.1 移除旧的数据源访问逻辑
...
- [ ] 4. 重构 MModUI.CreatePlayerEntry() 方法
  - [ ] 4.1 修改方法签名
  ...
  - [ ] 4.2 移除模式相关的显示逻辑
    ...
    - 移除 `isHost` 判断和前缀添加（HOST_、CLIENT_）
...
- [ ] 5. 重构 MModUI.UpdatePlayerPingDisplays() 方法
...
- [ ] 6. 清理和移除未使用的代码
  - [ ] 6.1 移除 GetSteamIdFromStatus() 方法
    - 删除整个方法定义
    - 确认没有其他地方调用此方法
    - _需求: 2.8_
